    ...
    def solve(self) -> bool:
        ...
        for col in range(9):
            for row in range(9):
                ...
                if field.is_finalized() is False:   # check if the field is empty
                    Game.remove_neighbours(self, domain, neighbours, field)
                    ...
                    if field.get_domain_size() == 1:
                        field.set_value(domain[0])  # if the domain has one value left, set the field to that value
